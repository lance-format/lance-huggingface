#!/usr/bin/env python3
"""Convert EuroSAT (blanchon/EuroSAT_RGB) to Lance.

Sentinel-2 RGB tiles (10 land-cover classes, 27k samples). Reuses the shared
...
HF_REPO_ID = "lance-format/eurosat-lance"
SOURCE_REPO = "blanchon/EuroSAT_RGB"


def main() -> None:
    p = argparse.ArgumentParser(description="EuroSAT RGB -> Lance")
    p.add_argument("--out", default=str(REPO_ROOT.parent / "lance_cache" / "eurosat-lance"))
    ...
    for split in ("train", "validation", "test"):
        ...
        # ClassLabel.names: "Annual Crop", "Forest", ... — sanitize spaces.
        class_names = [n.replace(" ", "_") for n in ds.features["label"].names]
